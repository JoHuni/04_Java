...
public class TodoListView {
	...
	public TodoListView() {
		...
		// 객체 생성 시 발생되는 예외는 여기서 모아서 처리
		try {
			// service == 부모타입인 TodoListService의 참조변수
	...
	public void startView() {
		...
		do {
			try {
				...
				input = -1; // 처음부터 잘못 입력했을 때 프로그램이 종료되는걸 방지
			}
			catch(IOException e) {
				System.out.println("### 입/출력 관련 예외 발생");
				e.printStackTrace(); // 어디서 예외났는지 추적
			}
	...
	private void todoAdd() throws IOException {
		...
		// 성공 : 추가된 인덱스 return, 실패 : -1 return;
		int index = service.todoAdd(title, sb.toString());
		if(index == -1) {
			System.out.println("### 추가 실패 ###");
		}
		else {
			System.out.printf("%d번 인덱스에 추가되었습니다.\n", index);
		}
	}
